    ...
    >
                  ...
                  {rideHistory.map((r, i) => (
                    ...
                    >
                      <span style={{ color: "var(--dim)" }}>
                        {new Date(r.date).toLocaleDateString(LANG_LOCALE[language], { day: "numeric", month: "short" })}
                      </span>
                      <span className="digit" style={{ color: "var(--text)", fontWeight: 700 }}>
                        {toDisplay(r.distance).toFixed(1)} {displayDistUnit}
                      </span>
                      <span style={{ color: "var(--dim)" }}>
                        {Math.round(toDisplay(r.avgSpeed))}/{Math.round(toDisplay(r.maxSpeed))} {displayUnit}
                      </span>
                    </div>
                  ))}
                </div>
      ...
      <div className="volt-layout">
        ...
        <div className="enter-left portrait-left-align" style={{ display: "flex", flexDirection: "column", gap: 14, flexShrink: 0 }}>
          ...
          {navActive && (
            <div style={{ fontSize: 11, color: "var(--accent)", fontWeight: 600, maxWidth: 160 }}>
              ➜ {routeSteps[navStepIndex] ? phraseForStep(routeSteps[navStepIndex], language) : ""}
            </div>
          )}
        ...
        <div className="enter-center" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          {mapOn ? (
            mapCoords ? (
              <iframe
                ...
                src={
                  navActive && destCoords
                    ? `https://maps.google.com/maps?saddr=${mapCoords.lat},${mapCoords.lon}&daddr=${destCoords.lat},${destCoords.lon}&output=embed`
                    : `https://maps.google.com/maps?q=${mapCoords.lat},${mapCoords.lon}&z=15&output=embed`
                }
              />
            ) : (
              <div
          ...
          {autoPaused && gaugeOn && (
            <div style={{ fontSize: 10, color: "#FFA53B", letterSpacing: 0.5, fontWeight: 600 }}>
              ⏸ {t.autoPausedLabel}
            </div>
          )}
        </div>
